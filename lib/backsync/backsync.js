...
function BackSync (connection) {
  ...
  function saveAbout (object_id, info) {
    ...
    let query = { 'sources.facebook.fanpages.id': object_id.toString() }
    let data = {
      'description': info.description || info.about,

      'location.address': info.single_line_address,
      'location.parking':info.parking,

      'company.founded': info.founded
    }
