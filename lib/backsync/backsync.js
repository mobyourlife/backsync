...
function BackSync () {
  ...
  function loop () {
    if (shouldDequeue()) {
      ...
      // Dequeue objects and map their respective requests
      let count = Math.min(DEQUEUE_LENGTH, queue.length)
      let requests = queue.splice(0, count).map((i) => {
        return fb[i.type](i.object_id)
      })

      // Perform a batch request
      fb.auth()
      .then(() => fb.batchRequest(requests))
      .then((info) => console.log(JSON.stringify(info, null, 2)), console.error)
    }
